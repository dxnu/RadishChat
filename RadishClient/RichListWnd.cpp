...
void CPage1::OnClick(TNotifyUI& msg)
{
	...
	else if(msg.pSender->GetName() == _T("down_new"))
	{
		...
		new_node->SetAttribute(L"height", L"45");
		...
		new_h_lay->SetFloat(false);
		new_h_lay->SetChildPadding(10);
		RECT r = { 3, 5, 3, 5 };
		new_h_lay->SetInset(r);
		...
		new_btn_1->SetName(L"down_ico");
		new_btn_1->SetFloat(false);
		r = { 0, 0, 0, 0 };
		new_btn_1->SetBorderSize(r);
		new_btn_1->SetAttribute(L"width", L"32");
		new_btn_1->SetMaxHeight(26);
		new_btn_1->SetBkImage(L"downlist_app.png");
		new_btn_1->SetNormalImage(L"file='downlist_run.png' dest='20,14,32,26'");
		...
		new_label->SetTextColor(0xFFAAAAAA);
		new_label->SetShowHtml(true);
		...
		new_label2->SetAttribute(L"align", L"right");
		new_label2->SetTextColor(0xFFAAAAAA);
		new_label2->SetShowHtml(true);
	/*	new_label3->SetAttributeList(
			_T("align=\"right\" text=\"0.00K/34.33M \" textcolor=\"#FFAAAAAA\" showhtml=\"true\""));*/
		new_label3->SetAttribute(L"align", L"right");
		new_label3->SetText(L"0.00K/34.33M");
		new_label3->SetTextColor(0xFFAAAAAA);
		new_label3->SetShowHtml(true);
...
LRESULT CRichListWnd::OnMouseHover(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if(pHover->GetName() == _T("down_ico"))
	{
		...
		((CButtonUI *)pHover)->SetNormalImage(L"file='downlist_pause.png' dest='15,9,32,26'");
	}
	return 0;
}
